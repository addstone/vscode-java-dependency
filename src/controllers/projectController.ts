...
export class ProjectController implements Disposable {
    ...
    public async createJavaProject() {
        const projectKinds: string[] = [ProjectKind.Simple];
        if (contextManager.getContextValue(Context.MAVEN_ENABLED)) {
            projectKinds.push(ProjectKind.Maven);
        }
        const choice: string | undefined = projectKinds.length === 1 ? projectKinds[0] : await window.showQuickPick(projectKinds);
        switch (choice) {
            case ProjectKind.Maven:
                await commands.executeCommand(Commands.JAVA_MAVEN_CREATE_PROJECT);
                break;
            case ProjectKind.Simple:
